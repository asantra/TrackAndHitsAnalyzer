//
// This is for electron+laser samples, only print the numbers in a text file, no root file, list of root files, no distinction in bunch crossing, not taking too much memory
...
void process_track_tree_draw_v7(const char *fnlist = 0, const char *commentstr = 0)
{
   gROOT->ProcessLineSync("#define __RUN_PROC_TREE__ 1");
   gROOT->ProcessLineSync(".L MHists.C+");
   gROOT->ProcessLine("#include \"process_track_tree_draw_v7.C\"");
...
int run_process_track_tree_draw(const char *fnlist = 0, const char *commentstr = 0)
{
  ...
  if (!fnlist) {
    std::cout << "Usage: root -l process_track_tree_draw_v7.C\'(\"file_with_list_of_mc_files\")\'\n";
    return -1;
  }
...
//     }
    ...
    for (size_t ii = 0; ii < ntrck; ++ii) {
        ...
        //       /// remove photon
        //       if(pdg!=22)
            trackFile << "0 " << pdg << " " << track_id << " " << det_id << " " << xx << " " << yy << " " << eneg << " " << ev_weight << " " << vtx_x << " " << vtx_y << " " << vtx_z << std::endl;
        }
    } //tracks loop
  } // tree loop
