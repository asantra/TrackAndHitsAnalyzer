...
def DrawHistsOneCanvas(FirstTH1, LegendName, PlotColor,xAxisName, yAxisName, xrange1down, xrange1up, yrange1down, yrange1up, CanvasName, yline1low, yline1up, drawline=False, logy=False, latexName='', latexName2 = '', latexName3='', leftLegend=False, doAtlas=False, doLumi=False, noRatio=False, do80=False, do59=False, drawPattern="", logz=False, logx=False):
   ...
   Tex2 = MakeLatex(0.34,0.80,latexName2)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Code to make TDR plots')
    parser.add_argument('-s', action="store", dest="sigFile", type=str, default="signalMC_e0gpc_3.0_trackInfo.root")
    parser.add_argument('-b', action="store", dest="bkgFile", type=str, default="ePlusLaserBackground_list_root_7671ee4c_First3BX_trackInfo.root")
    args = parser.parse_args()
    
    SetLuxeStyle()
    inputDir            = "/Users/arkasantra/arka/Sasha_Work/OutputFile"
    
    inSignalFile        = args.sigFile
    inBkgFile           = args.bkgFile
    sigRootFile         = TFile(inputDir+"/"+inSignalFile)
    bkgRootFile         = TFile(inputDir+"/"+inBkgFile)
    
    eachSigName         = inSignalFile.split('.root')[0].split('_')
    eachBkgName         = inBkgFile.split('.root')[0].split('_')
    suffixName          = "_".join(eachSigName[:-1])
    suffixName         += "_"
    suffixName         += "_".join(eachBkgName[:-1])
    outDir              = "DistributionPlotsTDR_"+suffixName
    ...
    baseHistogramNames["tracking_planes_background_track_x_gamma"]                           = ["#it{x} position [mm]", "Particles/BX", 0.0, 600.0, 2e-3, 1e7, True, "hist", False, False]
    baseHistogramNames["tracking_planes_background_track_x_electrons"]                       = ["#it{x} position [mm]", "Particles/BX", 0.0, 600.0, 2e-3, 1e7, True, "hist", False, False]
    baseHistogramNames["tracking_planes_background_track_x_positrons"]                       = ["#it{x} position [mm]", "Particles/BX", 0.0, 600.0, 2e-3, 1e7, True, "hist", False, False]
    baseHistogramNames["tracking_planes_signal_track_x_positrons"]                           = ["#it{x} position [mm]", "Particles/BX", 0.0, 600.0, 2e-3, 1e7, True, "hist", False, False]
    
    baseHistogramNames["tracking_planes_background_track_x_gamma_sumE"]                      = ["#it{x} position [mm]", "#sum E/BX [GeV]", 0.0, 600.0, 1e-4, 5e4, True, "hist", False, False]
    baseHistogramNames["tracking_planes_background_track_x_electrons_sumE"]                  = ["#it{x} position [mm]", "#sum E/BX [GeV]", 0.0, 600.0, 1e-4, 5e4, True, "hist", False, False]
    baseHistogramNames["tracking_planes_background_track_x_positrons_sumE"]                  = ["#it{x} position [mm]", "#sum E/BX [GeV]", 0.0, 600.0, 1e-4, 5e4, True, "hist", False, False]
    baseHistogramNames["tracking_planes_signal_track_x_positrons_sumE"]                      = ["#it{x} position [mm]", "#sum E/BX [GeV]", 0.0, 600.0, 1e-4, 5e4, True, "hist", False, False]
    ...
    for name in baseHistogramNames:
        ...
        for i in range(0,8):
            h = bkgRootFile.Get(name+"_"+str(i))
    ...
    for name in baseHistogramNames:
        ...
        for i in range(0,8):
            h = sigRootFile.Get(name+"_"+str(i))
            checkType = str(type(h))
            if (("TH1" in checkType) or ("TH2" in checkType)):
