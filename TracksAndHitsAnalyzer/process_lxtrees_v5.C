...
// here the track time is a constant and trackedep, working only for old CDR signal files

#ifndef __RUN_PROC_HITS_TREE__

void process_lxtrees_v5(const char *fnlist = 0, std::string bxNumber=0, std::string nameOfFile = 0)
{
   ...
   gROOT->ProcessLine("#include \"process_lxtrees_v5.C\"");
...
int process_hits_tree_draw(const char *fnlist = 0, std::string bxNumber=0, std::string nameOfFile = 0)
{
  ...
  std::string allpixoutname = std::string("HitBranchesForAllPix_")+foutname+std::string(".root");
...
//   TFile *fileOut = new TFile( allpixoutname.c_str(), "UPDATE");
//   fileOut->cd();
...
//   hitTree->Branch("evweight", &evweight, "evweight/D");
...
//   
//   fileOut->cd();
//   TTree *hitTreeOneChip = new TTree( "hitTreeOneChip", "A tree to save hits from one chip of stave 0");
//   hitTreeOneChip->Branch("event", &event, "event/I");
//   hitTreeOneChip->Branch("energy", &energy, "energy/D");
//   hitTreeOneChip->Branch("evweight", &evweight, "evweight/D");
//   hitTreeOneChip->Branch("time", &time, "time/D");
//   hitTreeOneChip->Branch("x", &x, "x/D");
//   hitTreeOneChip->Branch("y", &y, "y/D");
//   hitTreeOneChip->Branch("z", &z, "z/D");
//   hitTreeOneChip->Branch("detector", detector, "detector[10]/C");
//   hitTreeOneChip->Branch("pdg_code", &pdg_code, "pdg_code/I");
//   hitTreeOneChip->Branch("track_id", &track_id, "track_id/I");
//   hitTreeOneChip->Branch("parent_id", &parent_id, "parent_id/I");
...
//       const auto &trackedepv = hit.trackedep;
      ...
      if (det_id >= 1000 && det_id <= 1015) {
        ...
        for (int trkid : tridlist) {
          ...
          energy        = 0.0;//trackedepv.at(hittrckind);//trackedepv.at(vndx);
          time          = 12.0;//trtv.at(hittrckind);
          ...
          hitTree->Fill();
          if(det==0 && layer_id==0)hitTreeOneChip->Fill();
